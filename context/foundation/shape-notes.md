...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "all three compound — workflow friction, missing instant availability check, scattered data"
    - topic: "primary persona scope"
      decision: "parking facilities in general — a product for many operators"
    - topic: "product insight"
      decision: "commercial product the builder can sell to parking operators"
    - topic: "auth model"
      decision: "email + password login, two roles: Admin and Operator"
    - topic: "price calculation timing"
      decision: "at reservation creation, based on declared arrival/departure window"
  frs_drafted: 13
...
- Two operators submitting overlapping reservations simultaneously must not both succeed if only one spot is available (race condition protection at the booking level).
