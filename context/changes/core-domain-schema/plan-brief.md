...
- The partial unique index on `is_active = true` prevents more than one active pricing tier at the DB level, but does not prevent the _only_ active tier from being deactivated, leaving zero active tiers. S-02 must handle that UX case.
